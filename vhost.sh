...
add_virtual_server() {
    ...
    virtualmin create-domain --domain $domain_name --pass `openssl rand -base64 12` --ip $ip_address --unix --dir $website_dir --logdir $log_dir --smtp --smtp-port $smtp_port --default-website-port 81 --default-ssl-port 4443 --web --ssl --email --logrotate --spam --mysql 
